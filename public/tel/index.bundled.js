...
const newcontact = document.querySelector('.clear');
...
class PhoneBook {
    constructor() {
        this.states = {
            new: { value: 'new', displayDelBtn: false, status: 'Создание нового контакта' },
...
function renderView(newState, element) {
    ...
    const myTemplate = html`${repeat(contacts, (item) => item.fio, (item) => html`
        <tr data-timestamp="${item.id}"><td>${item.fio}</td><td>${item.phone}</td><td>${item.desc ? item.desc : ''}</td></tr>`)}`;
    render(myTemplate, tb);
}
